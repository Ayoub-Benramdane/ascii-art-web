...
func Ascii_Art(w http.ResponseWriter, r *http.Request) {
	if err := r.ParseForm(); err != nil { // ERROR 500
		http.Error(w, "Status Interal server 500", http.StatusInternalServerError)
		return
	}
	...
	if utf8.RuneCountInString(q.Text) > 1001 {
		q.Text = q.Text[:1001] + "..."
	}
